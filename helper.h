...
{
	long double minErrorPerVert;
	long double maxErrorPerVert;
	long double totalError;
	long double averageError;
	long double offset;
	long double radius;

	template<typename A, typename B, int numVerts>
	static VertexComparison Compare(
		std::array<std::pair<A, A>, numVerts> _correct,
		std::array<std::pair<B, B>, numVerts> _approx
	)
	{
		long double minErr = 1000000.f;
		long double maxErr = 0.f;
		long double totalError = 0.f;

		for (int i = 0; i < numVerts; i++)
		{
			long double errorX = abs(_correct[i].first - _approx[i].first);
			long double errorY = abs(_correct[i].second - _approx[i].second);
			long double minXY = std::min(errorX, errorY);
			long double maxXY = std::max(errorX, errorY);
			minErr = std::min(minXY, minErr);
			maxErr = std::max(maxXY, maxErr);
			totalError += errorX;
			totalError += errorY;
		}
		long double avgError = totalError / (long double)numVerts;

		VertexComparison vc;
		vc.averageError = avgError;
		vc.maxErrorPerVert = maxErr;
		vc.minErrorPerVert = minErr;
		vc.totalError = totalError;
...
struct ErrorDetails
{
	ErrorDetails(float min, float max,
